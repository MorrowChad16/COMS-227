...
public class Game {
	...
	public static void main(String[] args) {
		...
		// ConsoleUI.printGrid(g);
		// System.out.println(g.getCell(0, 3));
		// System.out.println(g.getSize());
		// System.out.println(g.getScore());
		/*
		 * Game g = new Game(5, new GameUtil(), new Random(42)); int[] arr = {1, 2, 3,
		 * 4, 5}; System.out.println("Before:"); ConsoleUI.printGrid(g);
		 * g.copyRowOrColumn(2, Direction.DOWN); g.updateRowOrColumn(arr, 2,
		 * Direction.LEFT); System.out.println("After:"); ConsoleUI.printGrid(g);
		 */

		Game g = new Game(4, new GameUtil(), new Random(42));
		int[][] test = { { 1, 2, 3, 1 }, { 0, 1, 3, 2 }, { 0, 2, 3, 0 }, { 0, 1, 2, 0 } };
	...
	public Game(int givenSize, GameUtil givenConfig, Random givenRandom) {
		...
		rand = givenRandom;
		games = new GameUtil();
		grid = games.initializeNewGrid(givenSize, rand);
	...
	public int[] copyRowOrColumn(int rowOrColumn, Direction dir) {
		...
		if (dir == Direction.LEFT) {
			for (int i = 0; i < grid.length; i++) {
				copy[i] = grid[rowOrColumn][i];
			}
		}
		if (dir == Direction.RIGHT) {
			for (int i = grid.length - 1; i >= 0; i--) {
				copy[j] = grid[rowOrColumn][i];
				j++;
			}
		}
		if (dir == Direction.UP) {
			for (int i = 0; i < grid.length; i++) {
				copy[i] = grid[i][rowOrColumn];
			}
		}
		if (dir == Direction.DOWN) {
			for (int i = grid.length - 1; i >= 0; i--) {
				copy[j] = grid[i][rowOrColumn];
				j++;
			}
		}
		return copy;
	...
	public TilePosition newTile() {
		if (lastMove != null) {
			didUndo = true;
			TilePosition k = games.generateRandomTilePosition(grid, rand, lastMove);
			k.setValue(nextTile);
			nextTile = games.generateRandomTileValue(rand);
			setCell(k.getRow(), k.getCol(), k.getValue());
			score = games.calculateTotalScore(grid);
			return k;
		}
		didUndo = false;
		return null;
